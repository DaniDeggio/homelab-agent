...
_NEWS_HINTS = {"news", "nyheter", "headlines", "breaking", "latest", "today", "idag", "ultime", "notizie"}
...
def rank_search_results(query: str, results: List[dict]) -> List[dict]:
    ...
    def news_quality_adjustment(title: str, snippet: str, url: str) -> float:
        ...
        if netloc in _TRUSTED_NEWS_DOMAINS:
            adjustment += 1.2
        if any(term in text for term in ("latest news", "breaking news", "daily coverage", "news from", "ultime notizie")):
            adjustment += 0.4
    ...
    for result in results:
        ...
        # Avoid zero division or negative issues, although sorting handles it
